...
export class EntidadSanitariaService {
  ...
  findAll(): Observable<EntidadSanitaria[]> {
    return this.http.get<EntidadSanitaria[]>(
      `${BASE_URL}/entidadSanitaria/todas-entidades`,
      { headers: this.headers }
    );
  }

  findById(id: number): Observable<EntidadSanitaria> {
    return this.http.get<EntidadSanitaria>(`${BASE_URL}/entidadSanitaria/${id}`, { headers: this.headers });
  }
}
